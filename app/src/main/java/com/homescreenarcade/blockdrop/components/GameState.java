...
import com.homescreenarcade.blockdrop.pieces.*;
...
public class GameState extends Component {
    ...
    private LocalBroadcastManager statusMgr;

    private GameState(BlockDropWallpaper ga) {
		super(ga);
		actions = 0;
        ...
        statusMgr = LocalBroadcastManager.getInstance(ga);
...
//		nextDropTime = host.getResources().getIntArray(R.array.intervals)[0];
	...
	public int getAutoDropInterval() {
		return dropIntervals[Math.min(currentLevel, maxLevel)];
	}
		...
		if(addScore != 0) {
            ...
            if (statusMgr != null) {
                Intent scoreIntent = new Intent(ArcadeCommon.ACTION_STATUS)
                        .putExtra(ArcadeCommon.STATUS_INCREMENT_SCORE, addScore);
                statusMgr.sendBroadcast(scoreIntent);
            }
        }
	}
	...
	public void finishTransition() {
		...
		activePieces[activeIndex].setActive(true);
		setNextDropTime(gameTime + dropIntervals[Math.min(currentLevel,maxLevel)]);
	...
	@Override
	public void reconnect(BlockDropWallpaper ga) {
		super.reconnect(ga);
    ...
    public void setLevel(int newLevel) {
        ...
        nextDropTime = host.getResources().getIntArray(R.array.intervals)[newLevel];
        clearedLines = 10*newLevel;

        if (statusMgr != null) {
            Intent statusIntent = new Intent(ArcadeCommon.ACTION_STATUS)
                    .putExtra(ArcadeCommon.STATUS_LIVES, 0)
                    .putExtra(ArcadeCommon.STATUS_LEVEL, newLevel);
            if (newLevel == 0) {
                statusIntent.putExtra(ArcadeCommon.STATUS_RESET_SCORE, true);
            }
	...
	public static GameState getInstance(BlockDropWallpaper ga) {
		if(instance == null)
			instance = new GameState(ga);
		return instance;
	}

	public static GameState getNewInstance(BlockDropWallpaper ga) {
		instance = new GameState(ga);
		return instance;
	}
